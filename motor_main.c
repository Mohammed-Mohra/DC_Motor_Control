...
int main(void)
{
	...
	uint32_t a = HAL_RCC_GetPCLK1Freq();
	tim_init();
...
void SystemClockConfig(uint8_t systick_clk_val)
{
	...
	case SYS_CLK_50MH:
		...
		rcc_clk_init.AHBCLKDivider = RCC_SYSCLK_DIV128;
		rcc_clk_init.APB1CLKDivider = RCC_HCLK_DIV16;
	...
	case hse_no_div:
		...
		rcc_clk_init.AHBCLKDivider = RCC_SYSCLK_DIV4;
